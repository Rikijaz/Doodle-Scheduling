...
export class AddEvent extends Component {
  ...
  onSubmitFirstPage = e => {
    ...
    if (this.state.title !== "" && this.state.title !== null) {
      //TODO: remove spaces
      localStorage.setItem("saved_title", JSON.stringify(this.state.title));
  ...
  submitEvent = e => {
    ...
    if (!editingEvent) {
      ...
      db.collection("events")
        .doc(JSON.parse(localStorage.getItem("currentUser"))) //TODO: change to event ID from possible cards
        .get()
        .then(doc => {
          if (doc.exists) {
      ...
      return (
        <div>
          <AddSecondPage
            ...
            goToThirdPage={(date, time, e) => this.goToThirdPage(date, time, e)}
          />
        </div>
      );
